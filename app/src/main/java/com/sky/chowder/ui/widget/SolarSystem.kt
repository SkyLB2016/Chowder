 ...
 * 卫星式菜单
 */
class SolarSystem @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : FrameLayout(context, attrs, defStyleAttr), OnClickListener {
    private var tempX = 1
    private var tempY = 1
    ...
    init {
        ...
        radius = TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_DIP, RADIUS.toFloat(), resources.displayMetrics).toInt()
        // 获取自定义属性的值
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        // 载入宽高
        setMeasuredDimension(width + paddingLeft + paddingRight,
    ...
    private fun getChildPadLeft(menuWidth: Int, lp: ViewGroup.MarginLayoutParams, childWidth: Int): Int {
        ...
        when (position) {
            CENTER, CENTER_TOP, CENTER_BOTTOM -> childPadLeft = (layoutWidth - childWidth) / 2 + lp.leftMargin + paddingLeft - paddingRight
            LEFT_TOP, LEFT_BOTTOM, CENTER_LEFT -> childPadLeft = paddingLeft + lp.leftMargin + (menuWidth - childWidth) / 2
            RIGHT_TOP, RIGHT_BOTTOM, CENTER_RIGHT -> childPadLeft = layoutWidth - childWidth - lp.rightMargin - paddingRight - (menuWidth - childWidth) / 2
        }
        return childPadLeft
    }

    //九个位置三行
    private fun getChildPadTop(menuHeight: Int, lp: ViewGroup.MarginLayoutParams, childHeight: Int): Int {
        var childPadTop = 0
        val layoutHeight = height
        val paddingTop = paddingTop
        when (position) {
            CENTER, CENTER_LEFT, CENTER_RIGHT -> childPadTop = (layoutHeight - childHeight) / 2 + lp.topMargin + paddingTop - paddingBottom
            LEFT_TOP, RIGHT_TOP, CENTER_TOP -> childPadTop = paddingTop + lp.topMargin + (menuHeight - childHeight) / 2
            LEFT_BOTTOM, CENTER_BOTTOM, RIGHT_BOTTOM -> childPadTop = layoutHeight - childHeight - lp.bottomMargin - paddingBottom - (menuHeight - childHeight) / 2
        }
        return childPadTop
    }
     ...
     * @param childCount
     */
    private fun setMenuListener(childCount: Int) {
    ...
    fun toggleMenu(Duration: Int) {
        ...
        for (i in 0..childCount - 1 - 1) {
            ...
            when (position) {
                ...
                -> {
                    ix = getCos(2f, childCount - 2, i) * tempX
                    iy = getSin(2f, childCount - 2, i) * tempY
                }
                CENTER//位于中间时的xy值，与LEFT_TOP一样，只是大了四倍而已,同时第一个与最后一个view连接，多了一个角度
                -> {
                    ix = getCos(1.toFloat() / 2, childCount - 1, i)
                    iy = getSin(1.toFloat() / 2, childCount - 1, i)
                }
                CENTER_TOP//平角时的xy值；默认为CENTER_TOP；CENTER_BOTTOM,CENTER_LEFT,CENTER_RIGHT
                    , CENTER_BOTTOM//与LEFT_BOTTOM一样，只是大了两倍而已
                -> {
                    ix = getCos(1f, childCount - 2, i)
                    iy = getSin(1f, childCount - 2, i) * tempY
                }
                CENTER_LEFT//弹出位置改变，sin与cos互换
                -> {
                    ix = getSin(1f, childCount - 2, i)
                    iy = getCos(1f, childCount - 2, i)
                }
                CENTER_RIGHT//与CENTER_LEFT的X相反，X*-1
                -> {
                    ix = getSin(1f, childCount - 2, i) * tempX
                    iy = getCos(1f, childCount - 2, i)
                }
            }
    ...
    private fun getCos(num: Float, count: Int, num3: Int): Float {
        return (radius * Math.cos(Math.PI / num.toDouble() / count.toDouble() * num3)).toFloat()
    }

    private fun getSin(num: Float, count: Int, num3: Int): Float {
        return (radius * Math.sin(Math.PI / num.toDouble() / count.toDouble() * num3)).toFloat()
    }
     ...
     * @param pos
     */
    protected fun childAnimator(pos: Int) {
     ...
     * @param i 时间
     */
    fun rotationMenu(v: View, f: Float, g: Float, i: Int) {
        ObjectAnimator.ofFloat(v, "rotation", f, g).setDuration(i.toLong()).start()
    }
